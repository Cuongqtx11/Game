...
function renderDailyMissionBox() {
  qs('#daily-missions').innerHTML = `<ul class="mini-list">${content.dailyMissions.map(m => `<li>${m}</li>`).join('')}</ul>`;
}
function renderContinueLearning() {
  const blocks = content.chapters.slice(0,2).flatMap(g => g.items.slice(0,2));
  qs('#continue-learning').innerHTML = `<ul class="mini-list">${blocks.map(i => `<li>${i.title} • ${i.lessons[0]}</li>`).join('')}</ul>`;
}
function renderDailyWordsBox() {
  qs('#daily-words-box').innerHTML = `<ul class="mini-list">${getDailyWords(8).map(w => `<li>${w.korean} — ${w.meaning}</li>`).join('')}</ul>`;
}
function renderSavedWordsBox() {
  const saved = content.vocabulary.filter(v => state.savedWords.includes(v.korean));
  qs('#saved-words-box').innerHTML = saved.length ? `<ul class="mini-list">${saved.slice(0,12).map(w => `<li>${w.korean} — ${w.meaning}</li>`).join('')}</ul>` : 'Chưa có từ nào được lưu.';
}
  ...
  qs('#chapter-groups').innerHTML = content.chapters.map(group => `
    <div class="chapter-group">
      ...
      <div class="chapter-items">
        ${group.items.map(item => `<article class="chapter-card card"><h3>${item.title}</h3><p>${item.summary}</p><p><strong>${item.lessons.length}</strong> bài • <strong>${item.quizCount}</strong> quiz</p><div class="lesson-tags">${item.tags.map(tag => `<span class="lesson-tag">${tag}</span>`).join('')}</div></article>`).join('')}
      </div>
    </div>`).join('');
}
...
function renderQuick7() {
  ...
  qs('#quick7-grid').innerHTML = content.quick7Days.map((item, idx) => `<article class="feature-card card"><h3>${item.day} - ${item.title}</h3><p>${item.focus}</p>${idx===todayIdx?'<p><strong>⭐ Gợi ý hôm nay</strong></p>':''}</article>`).join('');
}
function renderSpeak30() {
  const day = dayIndex(content.speak30Days.length);
  qs('#speak30-grid').innerHTML = content.speak30Days.map((item, idx) => `<article class="feature-card card"><h3>Day ${item.day}: ${item.title}</h3><p><strong>Từ khóa:</strong> ${item.keywords.join(', ')}</p><p><strong>Mẫu câu:</strong> ${item.phrase}</p>${idx===day?'<p><strong>🔥 Day gợi ý hiện tại</strong></p>':''}</article>`).join('');
}
